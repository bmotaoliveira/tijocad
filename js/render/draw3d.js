...
export function build3d(skipCameraReset=false){
  ...
  // ── Caixas elétricas (3D) — paralelepípedo embutido na parede ──
  {
    ...
    S.elecBoxes.forEach((arr, k)=>{
      ...
      arr.forEach(eb=>{
        ...
        edgeLine.userData.dyn=true; S.s3.add(edgeLine);
      });
    });
  }
  ...
  {
    const ptDepth = 0.03;
    [[S.hydroPoints, 0x1A7EC8, 0x0d4a7a],
     [S.gasPoints,   0xE8660A, 0x8B3A00]
      ...
      ptMap.forEach((arr,k)=>{
        ...
        arr.forEach(hp=>{
          ...
          mesh.renderOrder=3; mesh.userData.dyn=true; S.s3.add(mesh);
          const edgeGeo=new THREE.EdgesGeometry(geo);
          ...
          edgeLine.renderOrder=4; edgeLine.userData.dyn=true; S.s3.add(edgeLine);
        });
      });
    });
  }
  ...
  // Diâmetro do furo: ~6cm (tijolo 12,5) ou ~9cm (tijolo 15)
  {
    ...
    const holeR = (S.cellCm === 15 ? 0.045 : 0.030);  // raio do furo em metros
  ...
  // Canal: largura ≈ 60% de S.cellCm, altura ≈ 44% de S.brickHCm (proporcional ao 2D)
  {
  ...
  if(S.slopedKeys.size){
    ...
    S.slopedKeys.forEach(k => {
      ...
      if(dir === 'H'){
        ...
        const hPrev = (S.slopedKeys.has(kPrev) && br.has(kPrev)) ? getCellH(kPrev)/100 : hM;
        const hNext = (S.slopedKeys.has(kNext) && br.has(kNext)) ? getCellH(kNext)/100 : hM;
        hStart = (hPrev + hM) / 2;
        hEnd   = (hM + hNext) / 2;
      } else {
